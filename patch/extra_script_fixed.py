...
def build_microros(*args, **kwargs):
    ...
    def filter_cmake_flags(flags):
        result = []
        for f in flags:
            stripped = f.strip('"')
            # <> を含むフラグはshellがリダイレクトと誤認するため除外
            # （CHIP_ADDRESS_RESOLVE_IMPL_INCLUDE_HEADER 等、micro-ROSビルドに不要なESP-IDF固有フラグ）
            if '<' not in stripped and '>' not in stripped:
                result.append(stripped)
        return ' '.join(result)

    c_flags = "{} {} -DCLOCK_MONOTONIC=0 -D'__attribute__(x)='".format(
        filter_cmake_flags(env['CFLAGS']),
        filter_cmake_flags(env['CCFLAGS'])
    )
    cxx_flags = "{} {} -fno-rtti -DCLOCK_MONOTONIC=0 -D'__attribute__(x)='".format(
        filter_cmake_flags(env['CXXFLAGS']),
        filter_cmake_flags(env['CCFLAGS'])
    )
