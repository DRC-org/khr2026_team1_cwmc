...
const float LX = 0.1725f;  // [m] ロボット中心からホイールまでの前後距離
                           // (Machine center to wheel center X)
const float LY = 0.2425f;  // [m] ロボット中心からホイールまでの左右距離
                           // (Machine center to wheel center Y)
  ...
  for (uint8_t i = 0; i < 4; i++) {
    ...
    if (target_rpm[i] == 0) {
      ...
      command_current = static_cast<int32_t>(
          KP * error + KI * integral_errors[i] + KD * d_error);
    }
...
void update_odometry() {
  ...
  // 現在の角度(odom_theta)を使って、ローカル速度(vx,
  // vy)をグローバル移動量(delta_x, delta_y)に変換 回転行列: [ dx ] = [ cos -sin
  // ] [ vx ] [ dy ] = [ sin  cos ] [ vy ]
